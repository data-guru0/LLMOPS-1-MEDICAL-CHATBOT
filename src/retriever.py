...
# Define a strict custom prompt for the LLM
...
def set_custom_prompt():
    """Create and return a custom prompt template for RetrievalQA."""
    return PromptTemplate(template=CUSTOM_PROMPT_TEMPLATE, input_variables=["context", "question"])

def create_qa_chain():
    """Create a QA retrieval chain using the vector store and LLM."""
    try:
        ...
        qa_chain = RetrievalQA.from_chain_type(
            ...
            return_source_documents=False,  # Disables returning source documents in response
